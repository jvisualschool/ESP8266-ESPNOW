// ESP_B - ESP-NOW 6-in-1 Demo (Board B)
#include <Arduino.h>
...
bool myTurnToSend = false;  // B waits first

// Demo 3: Synced Counter
int32_t syncCounter = 0;
bool myTurnToCount = false;  // B waits first
unsigned long counterTimer = 0;
unsigned long counterInterval = 800;

// Demo 4: Seesaw (B receives offset from A)
int8_t seesawOffset = 0;

// Demo 5: Morse Code (B sends OK)
const char MORSE_SEQ[] = "---.-.";  // OK
int morseIdx = 0;
bool morseSending = false;  // B waits first
unsigned long morseTimer = 0;
const int MORSE_TICK = 300;
char morseBuf[12];
int morseBufLen = 0;

// Demo 7: Eyes (B receives from A)
int8_t eyeOffX = 0, eyeOffY = 0;
bool eyeBlinking = false;
int8_t eyePhase = 0;

// Demo 6: Snake (B waits for snake from A)
const int SNAKE_LEN = 7;
struct SnakeSeg { int16_t x, y; };
SnakeSeg snakeSegs[SNAKE_LEN];
bool snakeOnMe = false;  // B waits initially
int16_t snakeX = -50, snakeY = 37;
int8_t snakeVY = 1;
  ...
  switch (msg.msgType) {
    case MSG_DEMO_SYNC:
      ...
      if (currentDemo == 0) {
        ...
        ballX = -10; ballY = 32; ballVX = 2.5; ballVY = 1.5;
        ...
        morseIdx = 0; morseSending = false; morseBufLen = 0; morseTimer = millis();
    ...
    case MSG_EYE: {
      eyePhase = (int8_t)(msg.value / 100000);
      int32_t rem = msg.value % 100000;
      eyeOffX = (int8_t)(rem / 1000 - 20);
      eyeOffY = (int8_t)((rem % 1000) / 10 - 20);
      eyeBlinking = (rem % 10) == 1;
      break;
    }
...
void setup() {
  ...
  // Boot splash: B starts "NOW", A starts "ESP"
  const char* splashSeq[] = {"NOW", "ESP", "NOW"};
  for (int i = 0; i < 3; i++) {
...
void loop() {
  ...
  // B follows A; safety fallback if no sync received
  if (millis() - demoStartTime >= DEMO_DURATION + 2000) {
    currentDemo = (currentDemo + 1) % 7;
    demoStartTime = millis();
    if (currentDemo == 0) {
      eyeOffX = 0; eyeOffY = 0; eyeBlinking = false; eyePhase = 0;
    } else if (currentDemo == 1) {
      ballX = -10; ballY = 32; ballOnScreen = false; pingPongCount = 0;
      showSend = false; showRecv = false;
    } else if (currentDemo == 2) {
      myTurnToSend = false; receivedIcon = false; sendingIcon = false;
      iconScale = 0; iconTimer = millis();
...
void drawDemo4() {
  ...
  // A block (left end, outline)
  int ay = 38 + off;
  display.drawRect(4, ay, 10, 8, SSD1306_WHITE);
  display.setTextSize(1);
  display.setCursor(6, ay + 1);
  display.print("A");

  // B block (right end, filled = this board)
  int by = 38 - off;
  display.fillRect(114, by, 10, 8, SSD1306_WHITE);
  display.setTextColor(SSD1306_BLACK);
...
void drawDemo6() {
  if (snakeOnMe) {
    ...
    // Check exit right edge → send back to A
...
void drawDemo7() {
  if (eyePhase == 0) {
    drawEye(32, 38, eyeOffX, eyeOffY, eyeBlinking, 16, 7);
    drawEye(96, 38, eyeOffX, eyeOffY, eyeBlinking, 16, 7);
  } else {
    drawEye(64, 38, eyeOffX, eyeOffY, eyeBlinking, 24, 11);
  }
}
